...
def main():
    ...
    with open(args.log, 'w', encoding='utf-8') as log:
        ...
        for jams_path in jams_files:            # Generate output path
            ...
            if not os.path.exists(output_dir):
                os.makedirs(output_dir, exist_ok=True)
                  # Pass None as the output_path to let jams_to_jcrd.py use the Title_Artist naming format
            # It will save the file in the same directory as the input file, so we'll move it after
            output_path = None
            
            print(f"Converting {jams_path} -> {output_path}")
            log.write(f"Processing {jams_path}...\n")
              # Build command with optional arguments - let jams_to_jcrd.py choose the output filename
            # based on the Title_Artist format
            cmd = ["python", "jams_to_jcrd.py", jams_path]
            if args.meta:
                cmd.extend(["--meta", args.meta])
            if args.partition:
                cmd.extend(["--partition", args.partition])
                
            # We'll also use --out-dir to specify where to save the file
