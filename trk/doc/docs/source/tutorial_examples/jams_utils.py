...
def jams_converter(
    ...
    if multi_section_data is not None:
        ...
        for sections in multi_section_data:
            ...
            if not (
                isinstance(sections[0], list)
                and isinstance(sections[0][0], tuple)
            ):
                raise TypeError(
            ...
            jam.annotations.append(
                multi_sections_to_jams(sections[0], sections[1])
            )
...
def sections_to_jams(section_data, description=None):
    ...
    if section_data is not None:
        ...
        for inter, seg in zip(section_data.intervals, section_data.labels):
            jannot_seg.append(
                time=inter[0], duration=inter[1] - inter[0], value=seg
            )
    if description is not None:
...
def notes_to_jams(note_data, description):
    ...
    if note_data is not None:
        ...
        for beg, end, n in zip(
            note_data.intervals[:, 0],
            note_data.intervals[:, 1],
            note_data.pitches,
        ):
...
def tempos_to_jams(tempo_data, description=None):
    ...
    jannot_tempo.annotation_metadata = jams.AnnotationMetadata(
        data_source="mirdata"
    )
    if tempo_data is not None:
        if not isinstance(tempo_data, float) and not isinstance(
            tempo_data, int
        ):
            raise TypeError("Type should be float or int.")
        jannot_tempo.append(
            time=0, duration=0, confidence=1, value=tempo_data
        )
    if description is not None:
...
def events_to_jams(event_data, description=None):
    ...
    if event_data is not None:
        ...
        for beg, end, label in zip(
            event_data.intervals[:, 0],
            event_data.intervals[:, 1],
            event_data.events,
        ):
...
def lyrics_to_jams(lyric_data, description=None):
    ...
    if lyric_data is not None:
        ...
        for beg, end, lyric in zip(
            lyric_data.intervals[:, 0],
            lyric_data.intervals[:, 1],
            lyric_data.lyrics,
        ):
...
def tag_to_jams(tag_data, namespace="tag_open", description=None):
    ...
    jannot_tag.annotation_metadata = jams.AnnotationMetadata(
        data_source="mirdata"
    )
