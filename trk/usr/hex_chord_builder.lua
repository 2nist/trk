...
function hex_chord_builder.render_chord_row(ctx, idx, chord)
  ...
  -- Extension dropdown  local ext_changed = false
  reaper.ImGui_SetNextItemWidth(ctx, 60)
  if reaper.ImGui_BeginCombo(ctx, "##ext" .. idx, chord.extension or "(none)") then
    for _, ext in ipairs(EXTENSIONS) do      local is_selected = (ext == chord.extension)
      -- Make sure we don't pass an empty string to ImGui_Selectable
      local display_ext = ext
      if display_ext == "" then display_ext = "(empty)" end
  ...
  local _, new_duration_from_input, _, value_changed_flag_duration = reaper.ImGui_InputInt(ctx, "##duration" .. idx, current_duration_val, 1)
  ...
  if is_selected and reaper.ImGui_PopStyleColor then
...
function hex_chord_builder.draw(ctx, ui_state_param)
  ...
  reaper.ImGui_SameLine(ctx, reaper.ImGui_GetWindowWidth(ctx) - 150)
  ...
  -- Chord list
  reaper.ImGui_Separator(ctx)
  if reaper.ImGui_BeginChild(ctx, "chord_list", 0, 200, 1) then -- Changed 5th arg from true to 1
  ...
  reaper.ImGui_Separator(ctx)
  if reaper.ImGui_BeginChild(ctx, "suggestions_panel", 0, 0, 1) then -- Changed 5th arg from true to 1
