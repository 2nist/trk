...
-- Try to load all necessary modules with pcall to handle missing ones gracefully
pcall(function() config = require("config") end)

-- Load bit operations library (needed for table flags)
...
-- Try multiple possible paths for utility modules
success, utils_file = pcall(function() return require("utils.file") end)
if not success then
...
success, utils_reaper = pcall(function() return require("utils.reaper") end)
if not success then 
  success, utils_reaper = pcall(function() return require("utils.reaper_helpers") end)
  if not success then utils_reaper = {} end
end

-- Try multiple possible paths for UI modules
success, Theme = pcall(function() return require("ui.theme") end)
if not success then 
...
function ProjectManager.load_projects()
  ...
  -- Projects file path
  local projects_path = config.paths and config.paths.data and (config.paths.data .. "/projects.json") or "projects.json"
  
  -- Try to read projects file
  local content = utils_file.read_file(projects_path)
  if not content then
    -- Projects file doesn't exist or couldn't be read
    state.status_message = "No projects file found. Creating new one."
    state.last_status_time = os.time()
    return false
  end
  
  -- Try to parse JSON
  local success, projects = pcall(function() return utils_json.decode(content) end)
  if not success or not projects then
    -- JSON parsing failed
    state.status_message = "Failed to parse projects file"
    state.last_status_time = os.time()
    return false
  end
  ...
  state.status_message = "Projects loaded"
  state.last_status_time = os.time()
  return true
end
...
function ProjectManager.save_projects()
  -- Check if file operations module is available
  if not utils_file.write_file then
    state.status_message = "File operations module not available"
    state.last_status_time = os.time()
    return false
  end
  ...
  -- Try to encode projects as JSON
  local success, content = pcall(function() return utils_json.encode(state.projects) end)
  if not success or not content then
    -- JSON encoding failed
    state.status_message = "Failed to encode projects"
    state.last_status_time = os.time()
    return false
  end
  
  -- Try to write projects file
  success = utils_file.write_file(projects_path, content)
  if not success then
    -- File write failed
    state.status_message = "Failed to write projects file"
    state.last_status_time = os.time()
    return false
  end
...
function ProjectManager.create_project()
  -- Check if name is empty
  if state.new_project_name == "" then
    state.status_message = "Project name cannot be empty"
    state.last_status_time = os.time()
    return false
  end
  ...
  for i, project in ipairs(state.projects) do
    if project.name == state.new_project_name then
      state.status_message = "Project with this name already exists"
      state.last_status_time = os.time()
      return false
    end
  end
  ...
  -- Create new project
  local new_project = {
    name = state.new_project_name,
    description = state.new_project_description,
    songs = {}
  }
  ...
  state.status_message = "Project created"
  state.last_status_time = os.time()
  return true
end

-- Update project details
function ProjectManager.update_project()
  -- Check if a project is selected
  if not state.selected_project or state.selected_project < 1 or state.selected_project > #state.projects then
  ...
  -- Check if name is empty
  if state.edit_project_name == "" then
    state.status_message = "Project name cannot be empty"
    state.last_status_time = os.time()
    return false
  end
  ...
  for i, project in ipairs(state.projects) do
    if i ~= state.selected_project and project.name == state.edit_project_name then
      state.status_message = "Project with this name already exists"
      state.last_status_time = os.time()
      return false
    end
  ...
  -- Update project
  state.projects[state.selected_project].name = state.edit_project_name
  state.projects[state.selected_project].description = state.edit_project_description
  ...
  state.status_message = "Project updated"
  state.last_status_time = os.time()
  return true
end

-- Delete project
function ProjectManager.delete_project()
  -- Check if a project is selected
  if not state.selected_project or state.selected_project < 1 or state.selected_project > #state.projects then
    state.status_message = "No project selected"
    state.last_status_time = os.time()
  ...
  -- Remove project
  table.remove(state.projects, state.selected_project)
  
  -- Update selected project
  if #state.projects > 0 then
    state.selected_project = math.min(state.selected_project, #state.projects)
  else
    state.selected_project = nil
  end
  
  -- Clear selected song indices
  state.selected_song_indices = {}
  ...
  state.status_message = "Project deleted"
  state.last_status_time = os.time()
  return true
end

-- Add songs to project
function ProjectManager.add_songs(songs)
  -- Check if a project is selected
  if not state.selected_project or state.selected_project < 1 or state.selected_project > #state.projects then
    state.status_message = "No project selected"
    state.last_status_time = os.time()
  ...
  -- Check if songs is a table
  if type(songs) ~= "table" then
    state.status_message = "Invalid songs data"
    state.last_status_time = os.time()
    return false
  end
  ...
  for _, song in ipairs(songs) do
    ...
    local exists = false
    for _, existing in ipairs(project.songs) do
      if existing.path == song.path then
        exists = true
        break
      end
    end
    ...
    if not exists then
      table.insert(project.songs, song)
      count = count + 1
    end
  end
  ...
  state.status_message = count .. " song(s) added to project"
  state.last_status_time = os.time()
  return true
end

-- Remove songs from project
function ProjectManager.remove_songs()
  -- Check if a project is selected
  if not state.selected_project or state.selected_project < 1 or state.selected_project > #state.projects then
    state.status_message = "No project selected"
    state.last_status_time = os.time()
  ...
  if #state.selected_song_indices == 0 then
    state.status_message = "No songs selected"
    state.last_status_time = os.time()
    return false
  end
  
  -- Sort selected indices in descending order
  table.sort(state.selected_song_indices, function(a, b) return a > b end)
  ...
  for _, index in ipairs(state.selected_song_indices) do
    if index >= 1 and index <= #project.songs then
      table.remove(project.songs, index)
      count = count + 1
    end
  end
  ...
  state.status_message = count .. " song(s) removed from project"
  state.last_status_time = os.time()
  return true
end

-- Export project
function ProjectManager.export_project()
  -- Check if a project is selected
  if not state.selected_project or state.selected_project < 1 or state.selected_project > #state.projects then
    state.status_message = "No project selected"
    state.last_status_time = os.time()
  ...
  -- Generate filename
  local filename = project.name:gsub("[^%w]", "_")
  local extension = state.export_options.format == 1 and ".json" or ".csv"
  local export_path = (config.paths and config.paths.exports or "") .. "/" .. filename .. extension
  
  -- Export data
  local content
  
  if state.export_options.format == 1 then
    -- Export as JSON
    local export_data = {
      name = project.name,
    ...
    for _, song in ipairs(project.songs) do
      local song_data = {
        title = song.title,
        artist = song.artist,
        path = song.path
      ...
      if state.export_options.include_metadata then
        song_data.album = song.album
        song_data.key = song.key
        song_data.tempo = song.tempo
        song_data.time_signature = song.time_signature
        song_data.tags = song.tags
      end
      
      -- Include comments if requested
      if state.export_options.include_comments and song.comments then
        song_data.comments = song.comments
      end
      
      table.insert(export_data.songs, song_data)
    end
    
    -- Encode as JSON
    local success
    success, content = pcall(function() return utils_json.encode(export_data) end)
    if not success or not content then
      state.status_message = "Failed to encode export data"
      state.last_status_time = os.time()
      return false
    end
  else
    -- Export as CSV
    content = "Title,Artist,Path"
    
    if state.export_options.include_metadata then
      content = content .. ",Album,Key,Tempo,Time Signature,Tags"
    end
    
    if state.export_options.include_comments then
    ...
    for _, song in ipairs(project.songs) do
      content = content .. utils_csv_escape(song.title or "") .. ","
      content = content .. utils_csv_escape(song.artist or "") .. ","
      content = content .. utils_csv_escape(song.path or "")
      
      -- Include additional metadata if requested
      if state.export_options.include_metadata then
        content = content .. "," .. utils_csv_escape(song.album or "")
        content = content .. "," .. utils_csv_escape(song.key or "")
        content = content .. "," .. utils_csv_escape(song.tempo and tostring(song.tempo) or "")
        content = content .. "," .. utils_csv_escape(song.time_signature or "")
        content = content .. "," .. utils_csv_escape(table.concat(song.tags or {}, "; "))
      end
      
      -- Include comments if requested
      if state.export_options.include_comments then
        content = content .. "," .. utils_csv_escape(song.comments or "")
      end
  ...
  -- Write export file
  local success = utils_file.write_file(export_path, content)
  if not success then
    state.status_message = "Failed to write export file"
    state.last_status_time = os.time()
    return false
  end
  ...
  state.status_message = "Project exported to " .. export_path
  state.last_status_time = os.time()
  return true
end

-- Helper function to escape CSV values
function utils_csv_escape(value)
...
function ProjectManager.draw()
  -- Get theme colors
  local colors = Theme.colors or {}
  
  -- Calculate time since last status message
  local status_age = os.time() - state.last_status_time
  
  -- Draw project list panel
  reaper.ImGui_BeginChild(state.ctx, "project_list", 200, 0, true)
  
  -- Draw project list header
  if reaper.ImGui_Button(state.ctx, "New Project", -1, 0) then
    -- Show create dialog
  ...
  -- Draw project list
  for i, project in ipairs(state.projects) do
    local is_selected = state.selected_project == i
    
    if reaper.ImGui_Selectable(state.ctx, project.name, is_selected) then
  ...
  reaper.ImGui_BeginChild(state.ctx, "project_details", 0, 0, true)
  
  if state.selected_project and state.projects[state.selected_project] then
    local project = state.projects[state.selected_project]
    
    -- Project header
    reaper.ImGui_Text(state.ctx, "Project: " .. project.name)
    if project.description and project.description ~= "" then
      reaper.ImGui_TextWrapped(state.ctx, project.description)
    end
    reaper.ImGui_Separator(state.ctx)
    
    -- Project actions
    if reaper.ImGui_Button(state.ctx, "Edit Project") then
      -- Show edit dialog
    ...
    if reaper.ImGui_Button(state.ctx, "Delete Project") then
      -- Show confirm dialog
    ...
    if reaper.ImGui_Button(state.ctx, "Export Project") then
      -- Show export dialog
    ...
    reaper.ImGui_SameLine(state.ctx)
    local rv_filter, new_filter = reaper.ImGui_InputText(state.ctx, "##filter", state.filter_text)
    if rv_filter then
      state.filter_text = new_filter or ""
    end
    ...
    if reaper.ImGui_BeginTable(state.ctx, "songs_table", 4, flags) then
      ...
      for i, song in ipairs(project.songs) do
        ...
        if show then
          ...
          if reaper.ImGui_Selectable(state.ctx, song.title or "Unknown", is_selected, reaper.ImGui_SelectableFlags_SpanAllColumns()) then
            -- Toggle selection
...
function ProjectManager.draw_dialogs()
  ...
  if reaper.ImGui_BeginPopupModal(state.ctx, "Create Project", true, reaper.ImGui_WindowFlags_AlwaysAutoResize()) then
    ...
    local rv_name, new_name = reaper.ImGui_InputText(state.ctx, "##name", state.new_project_name)
    if rv_name then
      state.new_project_name = new_name or ""
    end
    
    reaper.ImGui_Text(state.ctx, "Description:")
    local rv_desc, new_desc = reaper.ImGui_InputTextMultiline(state.ctx, "##description", state.new_project_description, 300, 100)
    if rv_desc then
      state.new_project_description = new_desc or ""
    end
  ...
  if reaper.ImGui_BeginPopupModal(state.ctx, "Edit Project", true, reaper.ImGui_WindowFlags_AlwaysAutoResize()) then
    ...
    reaper.ImGui_Text(state.ctx, "Name:")
    local rv_name, new_name = reaper.ImGui_InputText(state.ctx, "##edit_name", state.edit_project_name)
    if rv_name then
      state.edit_project_name = new_name or ""
    end
    
    reaper.ImGui_Text(state.ctx, "Description:")
    local rv_desc, new_desc = reaper.ImGui_InputTextMultiline(state.ctx, "##edit_description", state.edit_project_description, 300, 100)
    if rv_desc then
      state.edit_project_description = new_desc or ""
    end
  ...
  if reaper.ImGui_BeginPopupModal(state.ctx, "Export Project", true, reaper.ImGui_WindowFlags_AlwaysAutoResize()) then
    ...
    -- Include options
    local rv_meta, include_meta = reaper.ImGui_Checkbox(state.ctx, "Include metadata (album, key, tempo, etc.)", state.export_options.include_metadata)
    if rv_meta then
      state.export_options.include_metadata = include_meta
    end
    
    local rv_comments, include_comments = reaper.ImGui_Checkbox(state.ctx, "Include comments", state.export_options.include_comments)
    if rv_comments then
      state.export_options.include_comments = include_comments
    end
