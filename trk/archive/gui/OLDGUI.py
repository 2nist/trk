...
class JCRDToolboxGUI:
    ...
    def run_tool(self, script_name, input_path, success_msg, error_msg):
        ...
        # Create a single file directory approach for key and roman numeral scripts
        if script_name in ["add_key_estimation", "add_roman_numerals"]:
            # Get directory containing just this file
            parent_dir = os.path.dirname(input_path)
            filename = os.path.basename(input_path)
            single_file_dir = os.path.join(parent_dir, "_temp_dir")

            # Create a temporary directory with just the target file
            os.makedirs(single_file_dir, exist_ok=True)
            temp_file_path = os.path.join(single_file_dir, filename)
            ...
            try:
                ...
                # Run the script on the single file directory
                cmd = [
                    sys.executable,
                    script_path,
                    "--directory",
                ...
                self.update_status(f"Running {script_name} on: {filename}")
                ...
                # Check if successful, then copy the modified file back
                if "✅" in result.stdout:
                    with open(temp_file_path, "r") as src:
                        updated_data = json.load(src)
                    with open(input_path, "w") as dst:
                        json.dump(updated_data, dst, indent=2)
                    self.update_status(f"✅ {success_msg}: {input_path}")
                    messagebox.showinfo("Success", result.stdout)
                else:
                    self.update_status(
                        f"⚠️ Script ran but no changes detected"
                    )
                    messagebox.showinfo(
                        "Warning",
                        f"Script ran but no changes detected:\n{result.stdout}",
                    )
            except Exception as e:
                error_details = str(e)
                if hasattr(e, "stderr"):
                    error_details = e.stderr or error_details
                ...
                try:
                    ...
                    if os.path.exists(single_file_dir):
                        os.rmdir(single_file_dir)
                except:
                    pass
        else:
            # Standard scripts
            cmd = [sys.executable, script_path, "--input", input_path]
    ...
    def install_module(self, module_name):
        ...
        msg = f"The {module_name} module is required for this operation.\n"
        msg += "Would you like to install it now?"
        if messagebox.askyesno("Install Dependency", msg):
            try:
                ...
                error_msg += "Please install it manually."
