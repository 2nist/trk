#!/bin/bash
# run_only_sketch_tests.sh - Run only the Sketch Mode tests (no ImGui tests)
...
echo "===== SKETCH MODE TEST SUITE ====="

# Set environment variable to skip ImGui tests
export SKIP_IMGUI_TESTS=1

# Run Lua with our test script that only tests Sketch Mode
lua -e "
...
all_passed = test_activate_sketch_mode() and all_passed
all_passed = test_new_sketch_section() and all_passed
all_passed = test_promote_to_arrangement() and all_passed
