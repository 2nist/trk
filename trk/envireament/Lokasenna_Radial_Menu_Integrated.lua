-- Lokasenna Radial Menu (Integrated for Card Stack Context)
-- This is a patched version for Songbase, with context-aware stack integration.
-- Original author: Lokasenna
-- Patch: Songbase integration (2025-06-11)

-- Context/Result integration helpers
local context_file = reaper.GetResourcePath() .. '/Scripts/songbase/radial_menu_context.json'
local result_file = reaper.GetResourcePath() .. '/Scripts/songbase/radial_menu_result.json'
...
local function read_context()
  if not file_exists(context_file) then return nil end
  local f = io.open(context_file, 'r')
  if not f then return nil end
  local content = f:read('*a')
  f:close()
  local ok, ctx = pcall(function() return load('return ' .. content:gsub('null', 'nil'))() end)
  if ok then return ctx else return nil end
end
...
-- write_result(ctx.stack)
-- This will notify the main session/arrangement logic

-- Example usage in your stack modification functions:
-- function add_card_to_stack(card)
--   table.insert(ctx.stack, card)
--   write_result(ctx.stack)
--[[
...
local function move_btn(dir)
	...
	-- Songbase integration: update stack result after move
	write_result(ctx.stack)
