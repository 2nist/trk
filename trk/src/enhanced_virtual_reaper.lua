...
function EnhancedVirtualReaper.create_environment()
  ...
  -- Integrate modular ImGui mocks (widget-level)
  local virtual_imgui = require("virtual_imgui")(VirtualState, log_api_call, log_warning, log_error)
  for name, fn in pairs(virtual_imgui) do
    mock_reaper["ImGui_" .. name] = fn
  end
  -- Provide builtin path for ImGui module demos
  mock_reaper.ImGui_GetBuiltinPath = function()
    return "/Users/Matthew/songbase/external/reaimgui-master"
  end
  -- Preload 'imgui' module to load virtual ImGui API for demo scripts
  package.preload["imgui"] = function()
    local M = require("virtual_imgui")(VirtualState, log_api_call, log_warning, log_error)
