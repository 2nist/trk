...
-- Function to load the next batch of files
local function load_next_batch()
  if not state.all_files or not state.current_batch then return false end
  
  -- Check if we've loaded everything
  if state.current_batch >= state.total_batches then
    state.loading = false
    reaper.ShowConsoleMsg("✓ All batches loaded: " .. state.successful_loads .. " songs loaded, " .. 
  ...
  local start_idx = ((state.current_batch - 1) * state.batch_size) + 1
  local end_idx = math.min(start_idx + state.batch_size - 1, #state.all_files)
  
  reaper.ShowConsoleMsg("Loading batch " .. state.current_batch .. "/" .. state.total_batches .. 
                       " (" .. start_idx .. "-" .. end_idx .. " of " .. #state.all_files .. " files)\n")
  
  -- Process this batch
  for i = start_idx, end_idx do
    local filepath = state.all_files[i]
    ...
    if file_ops.read_file then
      ...
      if content then
        ...
        if success and data then
          ...
          state.successful_loads = state.successful_loads + 1
        else
          state.failed_loads = state.failed_loads + 1
          if state.current_batch == 1 then -- Only log errors for first batch to avoid console spam
            reaper.ShowConsoleMsg("  Failed to parse JSON: " .. tostring(data) .. "\n")
          end
        end
      else
        state.failed_loads = state.failed_loads + 1
        if state.current_batch == 1 then
...
local function load_songs()
  ...
  -- Quick check: if directories don't exist, don't try to scan them
  local valid_paths = {}
  for _, path in ipairs(paths) do
    ...
    if file_ops.directory_exists then
      ...
      -- Try alternative method to check directory
      local test_file = io.open(path .. "/.test", "r")
      if test_file then
        test_file:close()
        exists = true
      end
    ...
    if exists then
      ...
      reaper.ShowConsoleMsg("✓ Will scan: " .. path .. "\n")
    else
      reaper.ShowConsoleMsg("⚠ Skipping non-existent: " .. path .. "\n")
    end
  end
  ...
  if state.scan_paths and #state.scan_paths > 0 then
    for _, path in ipairs(state.scan_paths) do
      table.insert(valid_paths, path)
      reaper.ShowConsoleMsg("✓ Custom path: " .. path .. "\n")
    end
  end
  
  if #valid_paths == 0 then
    state.loading = false
    state.error = "No valid directories found to scan"
    reaper.ShowConsoleMsg("❌ No valid directories found for scanning\n")
    return false
  end
  ...
  if #all_files == 0 then
    state.loading = false
    state.error = "No JCRD files found in scan directories"
    reaper.ShowConsoleMsg("⚠ No JCRD files found\n")
    return false
  end
  
  reaper.ShowConsoleMsg("Song Browser: Processing " .. #all_files .. " files...\n")
  
  -- Store all files for lazy loading
  state.all_files = all_files
  state.current_batch = 0
  state.batch_size = 10 -- Set a smaller batch size to prevent freezing
  state.total_batches = math.ceil(#all_files / state.batch_size)
  state.successful_loads = 0
  state.failed_loads = 0

  -- Load just the first batch to show something quickly
  load_next_batch()
  
  -- Function will return and UI will show progress as batches load
  ...
  for _, song in ipairs(state.songs) do
    ...
    -- Update stats will be handled as each batch completes
  -- Initial stats update
  state.loading = true
  state.loaded = true
  
  -- Log start of batch loading
  reaper.ShowConsoleMsg("Song Browser: Started loading " .. #all_files .. " files in batches\n")
...
function song_browser.init(ctx, ui_state)
  ...
  -- Don't load songs immediately - only on first draw or manual refresh
  -- This prevents the UI from hanging during startup
  state.loading = false
  state.loaded = false
  state.current_batch = 0
...
function song_browser.draw(ctx, ui_state)
  ...
  if not state.loaded and not state.loading then
    reaper.ImGui_Separator(ctx)
    reaper.ImGui_Text(ctx, "Click 'Load Datasets' to scan for songs")
    if reaper.ImGui_Button(ctx, "Load Datasets") then
      load_songs()
    end
    reaper.ImGui_Separator(ctx)
  elseif state.loading and state.current_batch and state.total_batches then
    -- Show loading progress
    reaper.ImGui_Separator(ctx)
    reaper.ImGui_Text(ctx, string.format("Loading batch %d of %d (%d%% complete)", 
                      state.current_batch, state.total_batches,
                      math.floor((state.current_batch / state.total_batches) * 100)))
      -- Progress indicator (text-based since ProgressBar may not be available)
    local progress_percent = state.current_batch / state.total_batches
    local bar_width = 20 -- characters
    local filled_chars = math.floor(progress_percent * bar_width)
    local bar_str = string.rep("█", filled_chars) .. string.rep("░", bar_width - filled_chars)
    reaper.ImGui_Text(ctx, bar_str .. " " .. math.floor(progress_percent * 100) .. "%")
    
    -- Stats so far
    reaper.ImGui_Text(ctx, string.format("Loaded %d songs (%d failed)",
                     state.successful_loads or 0, state.failed_loads or 0))
    
    -- Manual load more button
    if reaper.ImGui_Button(ctx, "Load More") then
      load_next_batch()
    end
    reaper.ImGui_SameLine(ctx)
    if reaper.ImGui_Button(ctx, "Pause Loading") then
      state.pause_loading = not state.pause_loading
      if not state.pause_loading then
        load_next_batch() -- Resume if unpausing
      end
    end
    reaper.ImGui_Separator(ctx)
  end
