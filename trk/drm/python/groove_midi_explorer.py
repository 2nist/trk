...
TEST_RESOURCES = os.path.join(REPO_ROOT, "tests", "resources", "mir_datasets")
MIDI_OUTPUT_DIR = os.path.join(REPO_ROOT, "data", "midi", "groove_midi")
JCRD_OUTPUT_DIR = os.path.join(REPO_ROOT, "data", "jcrd_library", "groove_midi")
METADATA_DIR = os.path.join(REPO_ROOT, "data", "metadata", "groove_midi")
INFO_CSV = os.path.join(TEST_RESOURCES, "groove_midi", "info.csv")
...
def load_metadata_from_csv():
    ...
    if not os.path.exists(INFO_CSV):
        logger.error(f"Error: info.csv not found at {INFO_CSV}")
    ...
    try:
        with open(INFO_CSV, 'r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                midi_path = os.path.join(TEST_RESOURCES, "groove_midi", row['midi_filename'])
...
def find_groove_midi_files():
    ...
    groove_path = os.path.join(TEST_RESOURCES, "groove_midi")
    if not os.path.exists(groove_path):
        logger.error(f"Error: Groove MIDI dataset not found at {groove_path}")
        return []
    
    midi_files = []
    for root, _, files in os.walk(groove_path):
        for file in files:
...
def main():
    ...
    parser.add_argument("--index-path", default=os.path.join(METADATA_DIR, "groove_midi_index.json"),
