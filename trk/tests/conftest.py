...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--dataset", type=str, default="", help="dataset to test locally"
    )
    parser.addoption(
        "--dataset-version",
        type=str,
...
def pytest_sessionfinish(session, exitstatus):
    if len(session.config.option.report_file) > 0:
        ...
        report += (
            "Running time: " + str(session.config.option.durations_min) + "\n"
        )
        passed_amount = sum(
            1 for result in session.results.values() if result.passed
        )
        failed_amount = sum(
        ...
        if os.path.isdir(os.path.dirname(file_destination)):
            ...
            print(
                "Folder {} does not exist".format(
                    os.path.dirname(file_destination)
                )
            )
