...
def test_fill_pitch_contour():
    ...
    t, f, v = guitarset._fill_pitch_contour(
        times, freqs, voicings, 8, 1, duration=7
    )
    assert np.array_equal(t, te)
    assert np.array_equal(f, fe)
    assert np.array_equal(v, ve)
...
def test_load_beats():
    ...
    track = dataset.track(default_trackid)
    assert np.allclose(
        track.beats.times, [0.50420168, 1.00840336, 1.51260504]
    )
...
def test_load_pitch_contour_no_data():
    ...
    with pytest.raises(
        ValueError,
        match="Pitch contour annotation not found in the JAMS file.",
    ):
...
def test_load_notes_no_data():
    ...
    with pytest.raises(
        ValueError,
        match="Note annotation or 'data' key not found in the JAMS file.",
    ):
...
def test_load_notes():
    ...
    assert np.allclose(
        track.notes["e"].intervals[:, 1],
        [1.2604598639455844, 1.7336798185940552, 2.0],
    )
    assert np.allclose(
