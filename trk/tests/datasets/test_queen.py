...
def test_track():
    ...
    assert audio.shape == (
        144384,
    ), "audio shape {} was not (144384,)".format(audio.shape)
...
def test_load_chords():
    ...
    assert np.array_equal(
        chord_data.intervals[:, 0], np.array([0.0, 0.459, 4.122])
    )
    assert np.array_equal(
        chord_data.intervals[:, 1], np.array([0.459, 4.122, 4.911])
    )
...
def test_load_key():
    ...
    assert np.array_equal(
        key_data.intervals[:, 1], np.array([83.139, 108.519])
    )
