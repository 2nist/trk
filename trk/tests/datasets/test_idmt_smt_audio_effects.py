...
def test_metadata():
    ...
    with pytest.raises(FileNotFoundError):
        dataset = idmt_smt_audio_effects.Dataset(
            fake_data_home, version="test"
        )
    ...
    dataset = idmt_smt_audio_effects.Dataset(
        corrupted_data_home, version="test"
    )
...
def test_download(httpserver):
    ...
    httpserver.serve_content(
        open(
            "tests/resources/download/IDMT-SMT-AUDIO-EFFECTS.zip", "rb"
        ).read()
    )
