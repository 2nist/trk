...
def test_load_f0():
    ...
    assert np.allclose(
        f0_data.times[:2], np.array([0.0, 0.002902]), atol=1e-5, rtol=0
    )
    assert len(f0_data.frequencies) == 1034
    assert np.allclose(
        f0_data.frequencies[:2], np.array([0.0, 0.0]), atol=1e-5, rtol=0
    )
    assert len(f0_data.voicing) == 1034
    assert np.allclose(
        f0_data.voicing[:2], np.array([0.0, 0.0]), atol=1e-5, rtol=0
    )
