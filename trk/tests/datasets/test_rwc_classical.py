...
def test_load_sections():
    ...
    assert np.array_equal(
        section_data.intervals[:, 0], np.array([0.29, 419.96])
    )
    assert np.array_equal(
        section_data.intervals[:, 1], np.array([46.14, 433.71])
    )
    assert np.array_equal(
        section_data.labels, np.array(["chorus A", "ending"])
    )
...
def test_load_beats():
    ...
    assert np.array_equal(
        beat_data.times,
        np.array([1.65, 2.58, 2.95, 3.33, 3.71, 4.09, 5.18, 6.28]),
    )
    assert np.array_equal(
        beat_data.positions, np.array([2, 1, 2, 1, 2, 1, 2, 1])
    )
