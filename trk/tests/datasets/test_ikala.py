...
def test_track():
    ...
    assert np.mean(np.abs(vocal[:8820])) < np.mean(
        np.abs(instrumental[:8820])
    )
...
def test_load_f0():
    ...
    assert math.isclose(
        f0_data.frequencies[1], 260.94640451888694, rel_tol=1e-12
    )
...
def test_load_lyrics():
    ...
    assert np.array_equal(
        lyrics_data_simple.intervals[:, 0], np.array([0.027, 0.232])
    )
    assert np.array_equal(
        lyrics_data_simple.intervals[:, 1], np.array([0.232, 0.968])
    )
    ...
    # check values
    assert np.array_equal(
        lyrics_data_pronun.intervals[:, 0], np.array([0.021, 0.571])
    )
    assert np.array_equal(
        lyrics_data_pronun.intervals[:, 1], np.array([0.189, 1.415])
    )
    assert np.array_equal(lyrics_data_pronun.lyrics, ["ASDF", "EVERYBODY"])
    assert np.array_equal(
        pronun_data.intervals[:, 0], np.array([0.021, 0.571])
    )
    assert np.array_equal(
        pronun_data.intervals[:, 1], np.array([0.189, 1.415])
    )
