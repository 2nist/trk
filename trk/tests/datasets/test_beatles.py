...
def test_track():
    ...
    assert audio.shape == (
        44100 * 2,
    ), "audio shape {} was not (88200,)".format(audio.shape)

    track = dataset.track("10212")
    assert (
        track.beats is None
    ), "expected track.beats to be None, got {}".format(track.beats)
...
def test_load_beats():
    ...
    assert (
        type(beat_data.times) == np.ndarray
    ), "beat_data.times is not an np.ndarray"
    assert (
    ...
    assert (
        beatles.load_beats(None) is None
    ), "load_beats(None) should return None"
...
def test_load_sections():
    ...
    assert np.array_equal(
        section_data.intervals[:, 0], np.array([0.000000, 0.465])
    )
    assert np.array_equal(
        section_data.intervals[:, 1], np.array([0.465, 14.931])
    )
