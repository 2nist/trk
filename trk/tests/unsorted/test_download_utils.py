...
def test_downloader(mocker, mock_path):
    ...
    # test info message
    download_utils.downloader(
        "a", index=index, info_message="I am a message!"
    )
    mocker.resetall()
...
def test_download_index_cases(mocker, mock_path):
    ...
    download_utils.downloader(
        "a", index=index, remotes=None, partial_download=None
    )
...
def test_downloader_with_server_file(httpserver):
    ...
    _clean(save_dir)
    download_utils.downloader(
        save_dir, index=index, remotes={"b": TEST_REMOTE}
    )
...
def test_download_from_remote_destdir(httpserver, tmpdir):
    ...
    expected_download_path = os.path.join(
        str(tmpdir), "subfolder", "remote.wav"
    )
...
def test_extractall_unicode(mocker, mock_download_from_remote, mock_unzip):
    zip_files = (
        "tests/resources/utfissue.zip",
        "tests/resources/utfissuewin.zip",
    )
    expected_files_all = (
    ...
    for zipf, expected_files in zip(zip_files, expected_files_all):
        ...
        download_utils.extractall_unicode(
            zfile, os.path.dirname("tests/resources/")
        )
        zfile.close()
