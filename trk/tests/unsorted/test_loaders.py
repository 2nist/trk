...
def test_dataset_attributes():
    for dataset_name in DATASETS:
        dataset = mirdata.initialize(
            dataset_name,
            os.path.join(TEST_DATA_HOME, dataset_name),
            version="test",
        )
        ...
        assert isinstance(
            dataset._index, dict
        ), "{}.DATA is not properly set".format(dataset_name)
        assert (
            isinstance(dataset._download_info, str)
        ...
        assert callable(
            dataset.download
        ), "{}.download is not a function".format(dataset_name)
...
def test_smart_open():
    for dataset_name in DATASETS:
        ...
        dataset_module = importlib.import_module(
            f"mirdata.datasets.{dataset_name}"
        )
...
def test_download(mocker):
    for dataset_name in DATASETS:
        ...
        for exp in expected_params:
            assert (
                exp[0] in params
            ), "{}.download must have {} as a parameter".format(
                dataset_name, exp[0]
            )
            assert (
        ...
        if dataset.remotes != {}:
            ...
            for key in dataset.remotes:
                ...
                if (
                    dataset_name in KNOWN_ISSUES
                    and key in KNOWN_ISSUES[dataset_name]
                ):
                    continue
                ...
                try:
                    ...
                    assert (
                        request.ok
                    ), "Link {} for {} does not return OK".format(
                        url, dataset_name
                    )
...
def test_load_methods():
    for dataset_name in DATASETS:
        ...
        load_methods = [
            getattr(dataset_module, m)
            for m in all_methods
            if m.startswith("load_")
        ]
...
def test_multitracks():
    ...
    for dataset_name in DATASETS:
        ...
        # test data home specified
        data_home = os.path.join(data_home_dir, dataset_name)
        dataset_specific = mirdata.initialize(
            dataset_name, data_home=data_home
        )
        try:
            mtrack_test = dataset_specific.MultiTrack(
                mtrack_id, data_home=data_home
            )
        except:
            assert False, "{}: {}".format(dataset_name, sys.exc_info()[0])
