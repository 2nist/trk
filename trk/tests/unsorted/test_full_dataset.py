...
def dataset(test_dataset, dataset_version):
    if test_dataset == "":
        ...
        raise ValueError(
            "{} is not a dataset in mirdata".format(test_dataset)
        )
    data_home = os.path.join(
        "tests/resources/mir_datasets_full", test_dataset
    )
    return mirdata.initialize(
        test_dataset, data_home, version=dataset_version
    )
...
def test_index(skip_remote, dataset):
    ...
    if "version" not in dataset._index.keys():
        raise NotImplementedError(
            "The top-level key 'version' is missing in the index"
        )
