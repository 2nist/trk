...
def run_track_tests(track, expected_attributes, expected_property_types):
    ...
    for prop in track_attr["cached_properties"] + track_attr["properties"]:
        ...
        if prop in expected_property_types:
            assert isinstance(
                getattr(track, prop), expected_property_types[prop]
            )
            ...
            assert (
                False
            ), "{} not in expected_property_types or expected_attributes".format(
                prop
            )
...
def test_md5(mocker):
    ...
    mocker.patch(
        "mirdata.validate.open", new=mocker.mock_open(read_data=audio_file)
    )
...
def test_validator(
    mocker, mock_validate_index, missing_files, invalid_checksums
):
