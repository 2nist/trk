...
def test_multitrack():
    ...
    mtrack_metadata_tidx = core.MultiTrack(
        ...
        index,
        core.Track,
        metadata_mtrack_index,
    )
    assert mtrack_metadata_tidx._multitrack_metadata == {
        "x": 1,
        "y": 2,
        "z": 3,
    }
...
def test_multitrack_unequal_len():
    ...
    target2 = mtrack.get_target(
        ["a", "b", "c"], average=False, enforce_length=False
    )
...
def test_multitrack_unequal_sr():
    class TestTrack(core.Track):
        ...
        @property
        def f(self):
            return np.random.uniform(-1, 1, (2, 100)), np.random.randint(
                10, 1000
            )
...
def test_dataset_splits():
    ...
    assert set(splits.keys()) == set(
        ["train", "validation", "test", "omitted"]
    )
